...
namespace bill;
class Model_Realex_Transaction_generic extends \AbstractModel {
    ...
    protected $hash_order1 = "timestamp.merchantid";
...
$this->basicXML."\n".
'<'.$this->owner->hash_func.'hash><?$hash?></'.$this->owner->hash_func.'hash>'."\n".
/*'<?$hash?>'."\n".*/
//$this->additionalXML."\n".
'</request>';
	}
	...
	function getRequest(){
		if($this->mandatory){
			throw new Exception_PaymentFailed("Mandatory fields are not specified: ".join(', ',$this->mandatory)." in ".get_class($this));
		}
...
/*
            if($this->code)
                throw Exception_PaymentFailed::c("Payment failed: ".$this->message)
